...
otp_futures: dict[int, asyncio.Future[str]] = {}
user_fetching = set()
...
async def handle_otp_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Agar user_id otp_futures me hai to is message ko OTP maan ke
    future complete kar dete hain.
    Baaki messages ko ignore.
    """
    user_id = update.effective_user.id

    # Sirf usi user ka text OTP treat karo jiske liye future pending hai
    if user_id not in otp_futures:
        return

    code = (update.message.text or "").strip()
    fut = otp_futures.pop(user_id, None)

    if fut and not fut.done():
        fut.set_result(code)
        await update.message.reply_text("✅ OTP received, verifying login...")

async def playwright_login_and_save_state(
    username: str,
    password: str,
    user_id: int,
    chat_id: int,
    ...
    async with async_playwright() as p:
        ...
        otp_locator = page.locator(
            ...
            'input[name="email"][type="text"], '   # check-your-email code field
        ...
        otp_url_like = any(
            ...
            for s in [
                ...
                "challenge",
                "checkpoint",
                "codeentry",          # /auth_platform/codeentry/...
                "/auth_platform/",
        ...
        if otp_count > 0 or otp_url_like:
            # Yaha hum OTP mangenge aur wait karenge
            logging.info(
                "[PLOGIN-ASYNC] OTP / challenge page detected for %s (url=%s, otp_count=%s)",
                username,
                current_url,
                otp_count,
            )
            ...
            otp_futures[user_id] = fut

            # Telegram pe user se OTP maango
            await bot.send_message(
                chat_id=chat_id,
                text="🔑 OTP detected! Enter the code we sent to your email / phone:"
            )

            # Yaha wait karega jab tak handle_otp_message future complete nahi karega
            otp_code = await fut
            logging.info("[PLOGIN-ASYNC] Got OTP from user %s", user_id)

            # OTP field ko dobara locate (safe side)
            ...
            if await otp_input.count() == 0:
                raise ValueError("ERROR_OTP_INPUT: OTP input box not found after prompt")

            await otp_input.click()
            await page.wait_for_timeout(300)
            await otp_input.fill("")
            await otp_input.type(otp_code, delay=random.randint(60, 140))

            # Continue / confirm button click
            continue_btn = page.locator('button:has-text("Continue"), button[type="submit"]').first
            await page.wait_for_timeout(400)
            await continue_btn.click()

            # Login complete hone ke liye thoda wait
            await page.wait_for_timeout(6000)
            current_url = page.url
            logging.info("[PLOGIN-ASYNC] After OTP submit URL=%s", current_url)

        # Agar yahan tak aa gaye to login OK (OTP ho ya direct)
        logging.info("[PLOGIN-ASYNC] No further OTP required, login looks successful. URL=%s", current_url)
...
async def plogin_get_password(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        state_file = await playwright_login_and_save_state(
    username,
    password,
    user_id,
    chat_id,
